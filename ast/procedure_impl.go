...
func newProcedureDeclaration(name string, block Block, scope sym.Scope[Declaration], index int) Declaration {
	return &ProcedureDeclarationNode{
		...
		Block:            block,
		Scope:            scope,
		ProcedureUsage:   make([]Expression, 0),
		TokenStreamIndex: index,
	}
}

// All usages of the procedure in expressions.
func (d *ProcedureDeclarationNode) Usage() []Expression {
	return d.ProcedureUsage
}
...
func (d *ProcedureDeclarationNode) String() string {
	return fmt.Sprintf(procedureFormat, sym.ProcedureEntry, d.Name, len(d.ProcedureUsage))
}
