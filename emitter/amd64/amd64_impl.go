...
// Directive to declare a global symbol in the assembly code so that it can be linked with other object files.
const globalDirective = ".global %v\n"
...
func (a *assemblyCodeUnit) Print(print io.Writer, args ...any) error {
	...
	if len(globals) > 0 {
		header += fmt.Sprintf(globalDirective, strings.Join(globals, ", "))
	}
