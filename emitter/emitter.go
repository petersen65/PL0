...
func newEmitter(cpu CentralProcessingUnit) Emitter {
	if cpu != Amd64 {
		panic(cor.NewGeneralError(cor.Emulator, failureMap, cor.Fatal, unsupportedCpuTarget, cpu, nil))
	}
...
func (e *emitter) Emit(module cod.Module) (text TextSection, err error) {
	...
	for i, l := iterator.First(), make([]string, 0); i != nil; i = iterator.Next() {
		...
		default:
			err = cor.NewGeneralError(cor.Emulator, failureMap, cor.Error, unknownIntermediateCodeOperation, i.Code.Operation, nil)
			return
		}
