...
type emitter struct {
	...
	assemblyCode     amd.AssemblyCodeUnit    // assembly code unit for the target platform
...
var (
	...
	dataTypeReturn = map[ic.DataType][]amd.Register{
		ic.Integer64:  {amd.Rax},
		ic.Integer32:  {amd.Eax},
		ic.Integer16:  {amd.Ax},
		ic.Integer8:   {amd.Al},
		ic.Float64:    {amd.Xmm0},
		ic.Float32:    {amd.Xmm0},
		ic.Unsigned64: {amd.Rax},
		ic.Unsigned32: {amd.Eax},
		ic.Unsigned16: {amd.Ax},
		ic.Unsigned8:  {amd.Al},
		ic.Boolean:    {amd.Al},
		ic.Character:  {amd.Eax},
		ic.String:     {amd.Rax, amd.Rdx},
	}
	...
	zeroMaskReturn = map[amd.Register]uint64{
		amd.Rax:  0,
		amd.Eax:  0,
		amd.Ax:   0x800000000000FFFF,
		amd.Al:   0x80000000000000FF,
		amd.Xmm0: 0,
		amd.Rdx:  0,
	}
)
...
func newEmitter(target TargetPlatform, intermediateCodeUnit ic.IntermediateCodeUnit) Emitter {
	...
	return &emitter{
		...
		assemblyCode:     amd.NewAssemblyCodeUnit(amd.Application),
...
func (e *emitter) GetAssemblyCodeUnit() amd.AssemblyCodeUnit {
...
func (e *emitter) allocateVariables(iterator ic.Iterator, labels []string) {
	...
	for j, offset := 0, int32(0); iterator.Peek(j) != nil; j++ {
		if iterator.Peek(j).Quadruple.Operation == ic.AllocateVariable {
			...
			offset = amd.Align(offset-byteSize, alignment)
		...
		if iterator.Peek(j+1) != nil && iterator.Peek(j+1).Quadruple.Operation != ic.AllocateVariable {
			...
			offset = amd.Align(offset, callStackAlignment)

			// grow the call stack downwards to provide space for all local variables int the activiation record (2GB maximum)
			e.assemblyCode.AppendInstruction(amd.Sub, labels,
				amd.NewRegisterOperand(amd.Rsp),
				amd.NewImmediateOperand(-offset))

			// zero out the allocated space for local variables in the activation record
			e.assemblyCode.AppendInstruction(amd.Cld, nil)
			e.assemblyCode.AppendInstruction(amd.Mov, nil, amd.NewRegisterOperand(amd.Rax), amd.NewImmediateOperand(int32(0)))
			e.assemblyCode.AppendInstruction(amd.Mov, nil, amd.NewRegisterOperand(amd.Rcx), amd.NewImmediateOperand(-offset/amd.QuadWordSize))
			e.assemblyCode.AppendInstruction(amd.Mov, nil, amd.NewRegisterOperand(amd.Rdi), amd.NewRegisterOperand(amd.Rsp))
			e.assemblyCode.AppendPrefixedInstruction(amd.Rep, amd.Stosq, nil)
...
func (e *emitter) prologue(labels []string) {
	...
	e.assemblyCode.AppendInstruction(amd.Push, labels, amd.NewRegisterOperand(amd.Rbp))
	e.assemblyCode.AppendInstruction(amd.Mov, nil, amd.NewRegisterOperand(amd.Rbp), amd.NewRegisterOperand(amd.Rsp))
}
...
func (e *emitter) epilogue(labels []string) {
	...
	e.assemblyCode.AppendInstruction(amd.Mov, labels, amd.NewRegisterOperand(amd.Rsp), amd.NewRegisterOperand(amd.Rbp))
	e.assemblyCode.AppendInstruction(amd.Pop, nil, amd.NewRegisterOperand(amd.Rbp))
}
...
func (e *emitter) setup(depth int32) {
	...
	if depth > 0 {
		...
		e.assemblyCode.AppendInstruction(amd.Call, nil, amd.NewLabelOperand(amd.CreateStaticLinkLabel))
	}
}
...
func (e *emitter) copyLiteral(dataType ic.DataType, value any, labels []string) {
	...
	case ic.Integer64:
		...
		e.assemblyCode.AppendInstruction(amd.MovAbs, labels,
			amd.NewRegisterOperand(amd.R10),
			amd.NewImmediateOperand(value.(int64)))

		// push the R10 register onto the call stack
		e.assemblyCode.AppendInstruction(amd.Push, nil, amd.NewRegisterOperand(amd.R10))

	case ic.Integer32:
		// push the 32-bit signed integer onto the call stack and sign-extend it to 64 bits
		e.assemblyCode.AppendInstruction(amd.Push, labels, amd.NewImmediateOperand(value.(int32)))
	...
	case ic.Integer16:
		...
		e.assemblyCode.AppendInstruction(amd.Push, labels, amd.NewImmediateOperand(int32(value.(int16))))

	case ic.Integer8:
		// push the 8-bit signed integer onto the call stack and sign-extend it to 64 bits
		e.assemblyCode.AppendInstruction(amd.Push, labels, amd.NewImmediateOperand(value.(int8)))

	case ic.Unsigned64:
		// move the 64-bit unsigned integer into the R10 register without sign extension
		e.assemblyCode.AppendInstruction(amd.MovAbs, labels,
			amd.NewRegisterOperand(amd.R10),
			amd.NewImmediateOperand(value.(uint64)))
	...
	case ic.Unsigned32:
		...
		e.assemblyCode.AppendInstruction(amd.Mov, labels,
			amd.NewRegisterOperand(amd.R10d),
			amd.NewImmediateOperand(value.(uint32)))
	...
	case ic.Unsigned16:
		...
		e.assemblyCode.AppendInstruction(amd.Mov, labels,
			amd.NewRegisterOperand(amd.R10d),
			amd.NewImmediateOperand(uint32(value.(uint16))))
	...
	case ic.Unsigned8:
		...
		e.assemblyCode.AppendInstruction(amd.Mov, labels,
			amd.NewRegisterOperand(amd.R10d),
			amd.NewImmediateOperand(uint32(value.(uint8))))
	...
	case ic.Float64:
		...
		e.assemblyCode.AppendInstruction(amd.MovAbs, labels,
			amd.NewRegisterOperand(amd.R10),
			amd.NewImmediateOperand(binaryRepresentationIEEE754))
	...
	case ic.Character:
		...
		e.assemblyCode.AppendInstruction(amd.Push, labels, amd.NewImmediateOperand(int32(value.(rune))))
	...
	case ic.Boolean:
		...
		if value.(bool) {
			e.assemblyCode.AppendInstruction(amd.Push, labels, amd.NewImmediateOperand(uint8(1)))
		} else {
			e.assemblyCode.AppendInstruction(amd.Push, labels, amd.NewImmediateOperand(uint8(0)))
		}
...
func (e *emitter) loadVariable(dataType ic.DataType, offset, depthDifference int32, labels []string) {
	...
	if depthDifference == 0 {
		...
		e.assemblyCode.AppendInstruction(amd.Mov, labels,
			amd.NewRegisterOperand(amd.Edi),
			amd.NewImmediateOperand(depthDifference))

		// follow the static link to determine the 'variables base' pointer of the correct lexical parent activation record
		e.assemblyCode.AppendInstruction(amd.Call, nil, amd.NewLabelOperand(amd.FollowStaticLinkLabel))
	...
	case ic.Integer64, ic.Unsigned64, ic.Float64:
		...
		e.assemblyCode.AppendInstruction(amd.Mov, labels,
			amd.NewRegisterOperand(amd.R10),
			amd.NewMemoryOperand(basePointer, amd.Bits64, offset))
	...
	case ic.Integer32, ic.Character:
		...
		e.assemblyCode.AppendInstruction(amd.Movsxd, labels,
			amd.NewRegisterOperand(amd.R10),
			amd.NewMemoryOperand(basePointer, amd.Bits32, offset))
	...
	case ic.Integer16:
		...
		e.assemblyCode.AppendInstruction(amd.Movsx, labels,
			amd.NewRegisterOperand(amd.R10),
			amd.NewMemoryOperand(basePointer, amd.Bits16, offset))
	...
	case ic.Unsigned16:
		...
		e.assemblyCode.AppendInstruction(amd.Movzx, labels,
			amd.NewRegisterOperand(amd.R10d),
			amd.NewMemoryOperand(basePointer, amd.Bits16, offset))
	...
	case ic.Integer8:
		...
		e.assemblyCode.AppendInstruction(amd.Movsx, labels,
			amd.NewRegisterOperand(amd.R10),
			amd.NewMemoryOperand(basePointer, amd.Bits8, offset))
...
func (e *emitter) storeVariable(dataType ic.DataType, offset, depthDifference int32, labels []string) {
	...
	e.assemblyCode.AppendInstruction(amd.Pop, labels, amd.NewRegisterOperand(amd.R10))
	...
	case ic.Integer64, ic.Unsigned64, ic.Float64:
		...
		e.assemblyCode.AppendInstruction(amd.Mov, nil,
			amd.NewMemoryOperand(basePointer, amd.Bits64, offset),
			amd.NewRegisterOperand(amd.R10))
	...
	case ic.Integer16, ic.Unsigned16:
		...
		e.assemblyCode.AppendInstruction(amd.Mov, nil,
			amd.NewMemoryOperand(basePointer, amd.Bits16, offset),
			amd.NewRegisterOperand(amd.R10w))
	...
	case ic.Integer8, ic.Unsigned8, ic.Boolean:
		...
		e.assemblyCode.AppendInstruction(amd.Mov, nil,
			amd.NewMemoryOperand(basePointer, amd.Bits8, offset),
			amd.NewRegisterOperand(amd.R10b))
...
func (e *emitter) negate(dataType ic.DataType, labels []string) {
	...
	case ic.Integer64, ic.Integer32, ic.Integer16, ic.Integer8:
		...
		e.assemblyCode.AppendInstruction(amd.Pop, labels, amd.NewRegisterOperand(amd.R10))
		e.assemblyCode.AppendInstruction(amd.Neg, nil, amd.NewRegisterOperand(amd.R10))
		e.assemblyCode.AppendInstruction(amd.Push, nil, amd.NewRegisterOperand(amd.R10))

	case ic.Float64:
		// move the 64-bit floating-point value (IEEE 754 double precision) to the XMM0 register
		e.assemblyCode.AppendInstruction(amd.Movsd, labels,
			amd.NewRegisterOperand(amd.Xmm0),
			amd.NewMemoryOperand(amd.Rsp, amd.Bits64, 0))

		// move the 64-bit floating-point sign bit mask into the R10 register
		e.assemblyCode.AppendInstruction(amd.Mov, nil,
			amd.NewRegisterOperand(amd.R10),
			amd.NewImmediateOperand(float64SignBitMask))

		// move the 64-bit floating-point sign bit mask into the XMM1 register
		e.assemblyCode.AppendInstruction(amd.Movq, nil,
			amd.NewRegisterOperand(amd.Xmm1),
			amd.NewRegisterOperand(amd.R10))
		...
		e.assemblyCode.AppendInstruction(amd.Xorpd, nil,
			amd.NewRegisterOperand(amd.Xmm0),
			amd.NewRegisterOperand(amd.Xmm1))

		// move the negated 64-bit floating-point value back onto the top of the call stack
		e.assemblyCode.AppendInstruction(amd.Movsd, nil,
			amd.NewMemoryOperand(amd.Rsp, amd.Bits64, 0),
			amd.NewRegisterOperand(amd.Xmm0))
...
func (e *emitter) odd(dataType ic.DataType, labels []string) {
	...
	case ic.Integer64, ic.Integer32, ic.Integer16, ic.Integer8:
		...
		e.assemblyCode.AppendInstruction(amd.Test, nil, amd.NewRegisterOperand(amd.R10), amd.NewImmediateOperand(uint64(1)))
...
func (e *emitter) arithmeticOperation(dataType ic.DataType, operation ic.Operation, labels []string) {
	...
	case ic.Integer64, ic.Integer32, ic.Integer16, ic.Integer8, ic.Unsigned64, ic.Unsigned32, ic.Unsigned16, ic.Unsigned8:
		...
		e.assemblyCode.AppendInstruction(amd.Pop, labels, amd.NewRegisterOperand(amd.R11))

		// pop the left-hand integer value from the call stack into the R10 register
		// note: all integer values must be correctly sign-extended to 64 bits before arithmetic operations
		e.assemblyCode.AppendInstruction(amd.Pop, nil, amd.NewRegisterOperand(amd.R10))
		...
		case ic.Plus:
			...
			e.assemblyCode.AppendInstruction(amd.Add, nil, amd.NewRegisterOperand(amd.R10), amd.NewRegisterOperand(amd.R11))

		case ic.Minus:
			// subtract the right-hand integer value in the R11 register from the left-hand integer value in the R10 register
			e.assemblyCode.AppendInstruction(amd.Sub, nil, amd.NewRegisterOperand(amd.R10), amd.NewRegisterOperand(amd.R11))

		case ic.Times:
			// multiply the left-hand integer value in the R10 register with the right-hand integer value in the R11 register
			e.assemblyCode.AppendInstruction(amd.Imul, nil, amd.NewRegisterOperand(amd.R10), amd.NewRegisterOperand(amd.R11))
		}
	...
	case ic.Float64:
		...
		e.assemblyCode.AppendInstruction(amd.Movsd, nil,
			amd.NewRegisterOperand(amd.Xmm0),
			amd.NewMemoryOperand(amd.Rsp, amd.Bits64, amd.PointerSize))
		...
		case ic.Plus:
			...
			e.assemblyCode.AppendInstruction(amd.Addsd, nil,
				amd.NewRegisterOperand(amd.Xmm0),
				amd.NewRegisterOperand(amd.Xmm1))
		...
		case ic.Minus:
			...
			e.assemblyCode.AppendInstruction(amd.Subsd, nil,
				amd.NewRegisterOperand(amd.Xmm0),
				amd.NewRegisterOperand(amd.Xmm1))
		...
		case ic.Times:
			...
			e.assemblyCode.AppendInstruction(amd.Mulsd, nil,
				amd.NewRegisterOperand(amd.Xmm0),
				amd.NewRegisterOperand(amd.Xmm1))
		...
		case ic.Divide:
			...
			e.assemblyCode.AppendInstruction(amd.Divsd, nil,
				amd.NewRegisterOperand(amd.Xmm0),
				amd.NewRegisterOperand(amd.Xmm1))
		}

		// remove the top element of the call stack (the right-hand value) by adjusting the stack pointer by 1 times the pointer size
		e.assemblyCode.AppendInstruction(amd.Add, nil,
			amd.NewRegisterOperand(amd.Rsp),
			amd.NewImmediateOperand(int32(amd.PointerSize)))
	...
	case ic.Float32:
		...
		e.assemblyCode.AppendInstruction(amd.Movss, nil,
			amd.NewRegisterOperand(amd.Xmm0),
			amd.NewMemoryOperand(amd.Rsp, amd.Bits32, amd.PointerSize))

		// the XMM0 and XMM1 registers are used to perform the arithmetic operation which leaves the result in the XMM0 register
		switch operation {
		case ic.Plus:
			// add the right-hand floating-point value in the XMM1 register to the left-hand floating-point value in the XMM0 register
			e.assemblyCode.AppendInstruction(amd.Addss, nil,
				amd.NewRegisterOperand(amd.Xmm0),
				amd.NewRegisterOperand(amd.Xmm1))

		case ic.Minus:
			// subtract the right-hand floating-point value in the XMM1 register from the left-hand floating-point value in the XMM0 register
			e.assemblyCode.AppendInstruction(amd.Subss, nil,
				amd.NewRegisterOperand(amd.Xmm0),
				amd.NewRegisterOperand(amd.Xmm1))

		case ic.Times:
			// multiply the right-hand floating-point value in the XMM1 register with the left-hand floating-point value in the XMM0 register
			e.assemblyCode.AppendInstruction(amd.Mulss, nil,
				amd.NewRegisterOperand(amd.Xmm0),
				amd.NewRegisterOperand(amd.Xmm1))

		case ic.Divide:
			// divide the left-hand floating-point value in the XMM0 register by the right-hand floating-point value in the XMM1 register
			e.assemblyCode.AppendInstruction(amd.Divss, nil,
				amd.NewRegisterOperand(amd.Xmm0),
				amd.NewRegisterOperand(amd.Xmm1))
		}
...
func (e *emitter) divideInteger(dataType ic.DataType, labels []string) {
	...
	case ic.Integer64, ic.Integer32, ic.Integer16, ic.Integer8:
		...
		// note: all integer values must be correctly sign-extended to 64 bits before arithmetic operations
		e.assemblyCode.AppendInstruction(amd.Pop, nil, amd.NewRegisterOperand(amd.Rax))
		...
		e.assemblyCode.AppendInstruction(amd.Cqo, nil)
		...
		e.assemblyCode.AppendInstruction(amd.Idiv, nil, amd.NewRegisterOperand(amd.R11))

		// push the result onto the stack (quotient)
		e.assemblyCode.AppendInstruction(amd.Push, nil, amd.NewRegisterOperand(amd.Rax))
	...
	case ic.Unsigned64, ic.Unsigned32, ic.Unsigned16, ic.Unsigned8:
		...
		e.assemblyCode.AppendInstruction(amd.Xor, nil, amd.NewRegisterOperand(amd.Rdx), amd.NewRegisterOperand(amd.Rdx))

		// divide the 128-bit dividend in RDX:RAX by the 64-bit divisor in R11 (unsigned division)
		// note: the quotient is stored in RAX and the remainder in RDX
		e.assemblyCode.AppendInstruction(amd.Div, nil, amd.NewRegisterOperand(amd.R11))
...
func (e *emitter) compare(dataType ic.DataType, labels []string) amd.ComparisonType {
	var comparisonType amd.ComparisonType
	...
	case ic.Integer64, ic.Integer32, ic.Integer16, ic.Integer8, ic.Character:
		comparisonType = amd.ComparisonIntegerSigned

	case ic.Unsigned64, ic.Unsigned32, ic.Unsigned16, ic.Unsigned8, ic.Boolean:
		comparisonType = amd.ComparisonIntegerUnsigned

	case ic.Float64, ic.Float32:
		comparisonType = amd.ComparisonFloat
	...
	case ic.Integer64, ic.Integer32, ic.Integer16, ic.Integer8, ic.Character, ic.Unsigned64, ic.Unsigned32, ic.Unsigned16, ic.Unsigned8, ic.Boolean:
		...
		e.assemblyCode.AppendInstruction(amd.Cmp, nil, amd.NewRegisterOperand(amd.R10), amd.NewRegisterOperand(amd.R11))
	...
	case ic.Float64:
		...
		e.assemblyCode.AppendInstruction(amd.Add, nil,
			amd.NewRegisterOperand(amd.Rsp),
			amd.NewImmediateOperand(int32(2*amd.PointerSize)))
		...
		e.assemblyCode.AppendInstruction(amd.Ucomisd, nil, amd.NewRegisterOperand(amd.Xmm0), amd.NewRegisterOperand(amd.Xmm1))
	...
	case ic.Float32:
		...
		//       it does not raise an exception for NaN values, but sets the CPU flags to indicate unordered comparisons
		e.assemblyCode.AppendInstruction(amd.Ucomiss, nil, amd.NewRegisterOperand(amd.Xmm0), amd.NewRegisterOperand(amd.Xmm1))
...
func (e *emitter) unconditionalJump(name string, labels []string) {
	e.assemblyCode.AppendInstruction(amd.Jmp, labels, amd.NewLabelOperand(name))
}

// Perform a conditional jump based on the CPU flags set by the previous comparison operation.
func (e *emitter) conditionalJump(comparisonType amd.ComparisonType, jump ic.Operation, name string, labels []string) {
	// Select the correct jump instruction based on comparison type and operation
	var opcode amd.OperationCode
	...
	case ic.JumpLess:
		...
		case amd.ComparisonIntegerUnsigned, amd.ComparisonFloat:
			// note: valid for unsigned integers and floats (CF=1)
			opcode = amd.Jb

		case amd.ComparisonIntegerSigned:
	...
	e.assemblyCode.AppendInstruction(opcode, labels, amd.NewLabelOperand(name))
}
...
func (e *emitter) callFunction(name string, depthDifference int32, labels []string) {
	...
	e.assemblyCode.AppendInstruction(amd.Call, nil, amd.NewLabelOperand(name))
}
	...
	if dataType.IsPointer() || dataType.IsReference() {
		...
		for _, register := range dataTypeReturn[dataType] {
			...
			case register.IsSse() && dataType.AsPlain() == ic.Float64:
				...
				e.assemblyCode.AppendInstruction(amd.Movsd, labels,
					amd.NewRegisterOperand(register),
					amd.NewMemoryOperand(amd.Rsp, amd.Bits64, 0))
			...
			case register.IsGeneralPurpose() && (dataType.IsInteger() || dataType.IsCharacter() || dataType.IsBoolean() || dataType.IsString()):
				...
				e.assemblyCode.AppendInstruction(amd.Pop, labels, amd.NewRegisterOperand(register))
			...
			if zeroMaskReturn[register] != 0 {
				e.assemblyCode.AppendInstruction(amd.And, nil,
					amd.NewRegisterOperand(register),
					amd.NewImmediateOperand(zeroMaskReturn[register]))
			}
		}
	}

	// return from the function
	e.assemblyCode.AppendInstruction(amd.Ret, nil)
}
