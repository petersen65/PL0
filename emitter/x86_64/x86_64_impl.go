...
// Named offsets for the start of DWARF sections.
const debugAbbrevSectionStartOffset = 0
const debugLineSectionStartOffset = 0
	...
	for _, dtd := range dstab.DataTypes {
		...
		case *cor.PointerDataType:
			...
			baseTypes = append(baseTypes, elf.NewDebuggingInformationEntry(
				...
				elf.DW_CODE_pointer_type,
				[]*elf.AttributeItem{
					elf.NewAttributeItem(elf.Long, elf.ToRelativeReference(elementTypeLabel, compilationUnitLabel)),
					elf.NewAttributeItem(elf.Byte, uint8(dt.Size())),
				},
			))
