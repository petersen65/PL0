...
type (
	...
	assemblyCodeUnit struct {
		...
		textSection      *elf.AssemblySection[*Instruction]          // text section with assembly instructions
	}
)
...
func newAssemblyCodeUnit(targetPlatform cor.TargetPlatform, outputKind OutputKind) AssemblyCodeUnit {
	return &assemblyCodeUnit{
		...
		roStrDescSection: elf.NewAssemblySection[*elf.ReadOnlyDataItem](
...
func (a *assemblyCodeUnit) Export(format cor.ExportFormat, print io.Writer) error {
	...
	case cor.Json:
		...
		if raw, err := json.MarshalIndent(struct {
			RoUtf32Section *elf.AssemblySection[*elf.ReadOnlyDataItem] `json:"rodata_utf32_section"`
			RoInt64Section *elf.AssemblySection[*elf.ReadOnlyDataItem] `json:"rodata_int64_section"`
			TextSection    *elf.AssemblySection[*Instruction]          `json:"text_section"`
