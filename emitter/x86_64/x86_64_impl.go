...
func newAssemblyCodeUnit(buildConfiguration plt.BuildConfiguration, debugInformation dbg.DebugInformation) AssemblyCodeUnit {
	unit := &assemblyCodeUnit{
		...
		// note: attributes SHF_MERGE and SHF_STRINGS are set and hence the mergeable entity size is set to 1 byte
...
func (u *assemblyCodeUnit) AppendRuntime() {
	...
	noIndex := tok.NoTokenStreamIndex
...
func (u *assemblyCodeUnit) Location(index int, debugger elf.Debugger, attributes ...string) *elf.Directive {
	...
	if index == tok.NoTokenStreamIndex || u.debugInformation == nil || !u.HasDebugInformation() || id == 0 {
		return nil
	}
...
func (u *assemblyCodeUnit) updateDebugInfoSection(dstab *dbg.DebugStringTable) {
	...
	for _, dtd := range dstab.DataTypes {
		...
		case *dbg.PointerDataType:
			...
			baseTypes = append(baseTypes, elf.NewDebuggingInformationEntry(
				...
				elf.DW_CODE_pointer_type,
				[]*elf.AttributeItem{
					elf.NewAttributeItem(elf.Long, elf.ToRelativeReference(elementTypeLabel)),
					elf.NewAttributeItem(elf.Byte, uint8(dt.Size())),
				},
			))
	...
	stringType := elf.NewDebuggingInformationEntry(
		...
		[]*elf.AttributeItem{
			...
			elf.NewAttributeItem(elf.Long, elf.ToRelativeReference(lengthMemberTypeLabel)),
			...
			elf.NewAttributeItem(elf.Long, elf.ToRelativeReference(dataPointerMemberTypeLabel)),
		...
		for _, cd := range fd.Constants {
			...
			subPrograms = append(subPrograms, elf.NewDebuggingInformationEntry(
				...
				[]*elf.AttributeItem{
					...
					elf.NewAttributeItem(elf.Short, uint16(line)),
					elf.NewAttributeItem(elf.Byte, uint8(column)),
					elf.NewAttributeItem(elf.Long, elf.ToRelativeReference(constantTypeLabel)),
		...
		for _, vd := range fd.Variables {
			...
			subPrograms = append(subPrograms, elf.NewDebuggingInformationEntry(
				...
				[]*elf.AttributeItem{
					...
					elf.NewAttributeItem(elf.Short, uint16(line)),
					elf.NewAttributeItem(elf.Byte, uint8(column)),
					elf.NewAttributeItem(elf.Long, elf.ToRelativeReference(variableTypeLabel)),
	...
	// the termination compilation unit entry is used to mark the end of the compilation unit's children
	termination := elf.NewDebuggingInformationEntry("", elf.DW_CODE_suppression, []*elf.AttributeItem{elf.NewAttributeItem(elf.Byte, uint8(0))})
	...
	// add the termination entry to the .debug_info section
	u.DebugInfoSection.Append(termination)
}
