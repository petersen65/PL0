...
type (
	...
	ElfSection[T fmt.Stringer] struct {
		...
		EntitySize int                `json:"entity_size"` // size in bytes of one mergeable entity (required for SHF_MERGE and SHF_STRINGS)
...
func NewSection[T fmt.Stringer](directives []DirectiveKind, attributes []SectionAttribute, alignment, size int, offsets bool) *ElfSection[T] {
	return &ElfSection[T]{Directives: directives, Attributes: attributes, Alignment: alignment, EntitySize: size, Offsets: offsets, Content: make([]T, 0)}
}
